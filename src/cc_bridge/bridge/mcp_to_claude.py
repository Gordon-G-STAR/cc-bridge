...
@mcp.tool(
    ...
    description=(
        ...
        "- timeout_seconds：单次调用超时秒数（10–1800）；长任务调大可避免默认 300s 把仍在跑的 Claude 切断。\n\n"
...
async def claude_handoff(
    ...
    try:
        ...
        # PR7:主 agent 若【可证明零副作用】且合同允许,透明 failover 到 Codex。
        ...
        append_audit_record(
            ...
            extra={
                ...
                "failover": final.agent_used not in (None, "claude"),
            },
        )
        return final
    except LockBusy:
...
@mcp.tool(
    name="claude_handoff_cancel",
    description="取消异步 Claude handoff；杀掉 runner 进程树并标记 interrupted，永不抛异常。",
)
async def claude_handoff_cancel(handoff_id: str) -> dict:
    try:
