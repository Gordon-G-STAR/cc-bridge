...
from cc_bridge.bridge.evidence import baseline, classify_changes, gather
...
def test_baseline_gather_flags_new_file_outside_writable_scope(tmp_path):
    repo = tmp_path / "repo"
    _init_git_repo(repo)
    ...
    result = gather(repo, before, writable_paths=())

    assert result.verified_files == []
    assert result.scope_violations == ["src/new.py"]
    assert result.unverifiable == []
    assert result.evidence_level == "verified"
    assert result.reasons == {"src/new.py": "outside granted writable scope"}
